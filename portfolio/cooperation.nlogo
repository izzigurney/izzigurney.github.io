...
breed [cooperative-peoples cooperative-people]
breed [rebellious-peoples rebellious-people]
...
to setup-peoples
  set-default-shape turtles "person"   ;; applies to both breeds
  create-turtles initial-people [
    setxy random-xcor random-ycor
    set energy shrewdness * 4
    ifelse (random-float 1.0 < cooperative-probability) [
      set breed cooperative-peoples
      set color pink - 1.5
    ] [
      set breed rebellious-peoples
      set color sky - 2
    ]
  ]
end
...
to grow-resource  ;; patch procedure
  ifelse ( resource >= 8) [
    if lying-ability >= random-float 100
      [ set resource resource + 1 ]
  ][
    if cruelty >= random-float 100
      [ set resource resource + 1 ]
  ]
  if resource > 10
    [ set resource 10 ]
...
to color-resource  ;; patch procedure
  set pcolor (white)
end

to move  ;; turtle procedure
  rt random 360
  fd %-of-people-who-hate-the-prince
  set energy energy - shrewdness
  if energy < 0 [ die ]
end

to use  ;; turtle procedure
  ifelse breed = cooperative-peoples [
    use-cooperative
  ] [
    if breed = rebellious-peoples
      [ use-rebellious ]
  ]
end

to use-cooperative  ;; turtle procedure
  if resource > 8 [
    set resource resource - 1
...
to use-rebellious  ;; turtle procedure
  if resource > 0 [
    set resource resource - 1
    set energy energy + 50
  ]
...
"" ""
PENS
"rebel" 1.0 0 -13791810 true "" "plot count rebellious-peoples"
"cooperative" 1.0 0 -1664597 true "" "plot count cooperative-peoples"
